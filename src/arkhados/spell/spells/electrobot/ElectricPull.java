...
public class ElectricPull extends Spell {

    {
        iconName = "MindPoison.png";
    }

    public ElectricPull(String name, float cooldown, float range,
            float castTime) {
        super(name, cooldown, range, castTime);
    }

    public static ElectricPull create() {
        final float cooldown = 7.5f;
        final float range = 100f;
        final float castTime = 0.4f;

        final ElectricPull spell = new ElectricPull("Electric Pull", cooldown,
                range, castTime);

        spell.castSpellActionBuilder = (Node caster, Vector3f vec)
                -> new ACastOnGround(world, spell);

        spell.nodeBuilder = new ElectricPullBuilder();

        return spell;
    }
}

class ElectricPullBuilder extends AbstractNodeBuilder {
    ...
    private static final class AFinish extends EntityAction {
        ...
        public boolean update(float tpf) {
            Vector3f botLoc = cAreaEffect.getOwnerInterface()
            ...
            for (SpatialDistancePair target : spatials) {
                Vector3f dir = botLoc.subtract(target.spatial
    ...
    private ParticleEmitter createBlack(float radius, float delay) {
        ...
        purple.setImagesY(2);
        purple.setSelectRandomImage(true);
        purple.setStartColor(ElectricPullBuilder.color);
        purple.setEndColor(ElectricPullBuilder.color);
